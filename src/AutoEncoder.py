    ...
    for epoch in range(num_epochs):
        ...
        if do_train:
            ...
            for geno_data in geno_train_set_loader:
                ...
                y_true = [True if x >= 0.5 else False for x in geno_data.numpy()]
                y_pred = [True if x >= 0.5 else False for x in output.cpu().detach().numpy()]
                tp = np.count_nonzero(y_true)
                fp = np.count_nonzero([(bool(y) and not(bool(x))) for x, y in zip(y_true, y_pred)])
